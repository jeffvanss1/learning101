...
test('player like button + threaded episode rows (long seasons)', async () => {
  ...
  assert.equal((cat.match(/renderEpisodeGrid\(epGrid/g) || []).length, 5, 'component def + detail + anime detail + room modal (tv + anime branches)');
  ...
  assert.match(cat, /\{ \.\.\.video, type: 'anime', isAnime: true, anilistId: anilistId, malId: malId \}, \{ episode: n \}\)/, 'anime pick = absolute episode + malId, season NEVER passed (S14E5 bug), artwork spread-preserved');
